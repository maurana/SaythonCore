...
/// <summary>
/// Saython CRUD Base Class
/// </summary>
namespace Saython.Framework.CRUD
{
    public class SaythonCRUDBase
    {
        public SaythonCRUDBase()
        {

        }

        //Generate Primary Key for WHERE CLAUSE ...
        protected static SaythonDbFieldKey[] GeneratePrimaryKeys(string[] paramNames, Object[] paramValues, SaythonDbFieldType[] paramTypes, SaythonDbOperatorType[] paramSearch)
        {
            ...
            SaythonDbFieldKey[] pks = new SaythonDbFieldKey[paramNames.Length];
            Int16 idx = 0;
            foreach (Object param in paramNames)
            {
                ...
                if (paramSearch == null)
                    pks[idx] = new SaythonDbFieldKey(paramNames[idx], Convert.ToString(paramValues[idx]), paramTypes[idx]);
                else
                    pks[idx] = new SaythonDbFieldKey(paramNames[idx], Convert.ToString(paramValues[idx]), paramTypes[idx], paramSearch[idx]);
        ...
        protected static SaythonDbFieldKey[] GeneratePrimaryKeys(string[] paramNames, Object[] paramValues, SaythonDbFieldType[] paramTypes)
        {
            return GeneratePrimaryKeys(paramNames, paramValues, paramTypes, null);
        }

        //This function returns where clause string
        public static string BuildSqlWhere(SaythonDbFieldKey[] keys)
        {
            string whereClause = "";
            if (keys != null)
            {
                ...
                for (Int32 i = 0; i < keys.Length; i++)
                {
                    if (keys[i] != null)
                    {
                        if (keys[i].Value == "null") keys[i].OperatorType = SaythonDbOperatorType.Equal;
                        string opr = "=";
                        bool _LikeOrNotLike = false;
                        switch (keys[i].OperatorType)
                        {
                            case SaythonDbOperatorType.Equal: opr = "="; break;
                            case SaythonDbOperatorType.NotEqual: opr = "<>"; break;
                            case SaythonDbOperatorType.LessThan: opr = "<"; break;
                            case SaythonDbOperatorType.GreaterThan: opr = ">"; break;
                            case SaythonDbOperatorType.LessThanOrEqual: opr = "<="; break;
                            case SaythonDbOperatorType.GreaterThanOrEqual: opr = ">="; break;
                            case SaythonDbOperatorType.Like:
                                opr = "LIKE";
                                _LikeOrNotLike = true;
                                break;
                            case SaythonDbOperatorType.NotLike:
                                opr = "NOT LIKE";
                                _LikeOrNotLike = true;
                                break;
                        ...
                        if (!_LikeOrNotLike)
                        {
                            if (keys[i].Type == SaythonDbFieldType.Text)
        ...
        public static string BuildSqlOrderBy(SaythonDbFieldSort[] sorts)
        {
                ...
                if (sorts.Length > 0)
                {
                    for (int i = 0; i < sorts.Length; i++)
                    {
                        if (sorts[i] != null)
                        {
                            ...
                            {
                                string sortitem = (i == 0 ? " ORDER BY " : ", ") + sorts[i].Name + " " + Enum.GetName(typeof(SaythonDbSortDirection), sorts[i].Direction);
        ...
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { null }, new SaythonDbFieldSort[1] { null }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... ORDER BY
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldSort sort, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { null }, new SaythonDbFieldSort[1] { sort }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... ORDER BY
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldSort[] sorts, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { null }, sorts, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ...
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey key, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { key }, new SaythonDbFieldSort[1] { null }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ...
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey[] keys, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, keys, new SaythonDbFieldSort[1] { null }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ... ORDER BY
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey key, SaythonDbFieldSort sort, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { key }, new SaythonDbFieldSort[1] { sort }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ... ORDER BY
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey key, SaythonDbFieldSort[] sorts, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, new SaythonDbFieldKey[1] { key }, sorts, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ... ORDER BY
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey[] keys, SaythonDbFieldSort sort, bool IsSelectAll)
        {
            return BuildSqlSelect(item, tableName, keys, new SaythonDbFieldSort[1] { sort }, IsSelectAll);
        }

        //This function returns sql string: SELECT ... FROM ... WHERE ... ORDER BY ...
        public static string BuildSqlSelect(Object item, string tableName, SaythonDbFieldKey[] keys, SaythonDbFieldSort[] sorts, bool IsSelectAll)
        {
            SaythonDbField attr;
            ArrayList fldList;
            string fieldClause = "";
        ...
        public static string BuildSqlUpdate(Object businessObject, string tableName, SaythonDbFieldKey key)
        {
            return BuildSqlUpdate(businessObject, tableName, new SaythonDbFieldKey[1] { key });
        }

        //This function returns sql string: UPDATE ... WHERE ...
        public static string BuildSqlUpdate(Object businessObject, string tableName, SaythonDbFieldKey[] keys)
        {
            ArrayList list;
            string fieldClause = "";
            Int32 p = 1;
        ...
        public static string BuildSqlDelete(string tableName, SaythonDbFieldKey key)
        {
            return BuildSqlDelete(tableName, new SaythonDbFieldKey[1] { key });
        }

        //This function returns sql string: DELETE ... WHERE
        public static string BuildSqlDelete(string tableName, SaythonDbFieldKey[] keys)
        {
        ...
        public static void FillProperties(Object businessObject, DataRow dataRow)
        {
            ...
            try
            {
                ...
                {
                    if (fieldInfos[i].IsDefined((new SaythonDbField()).GetType(), false))
                    {
                        errField = fieldInfos[i];
                        fieldAttr = (SaythonDbField)fieldInfos[i].GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
            ...
            try
            {
                ...
                {
                    if (propInfos[i].IsDefined((new SaythonDbField()).GetType(), false))
                    {
                        errProp = propInfos[i];
                        fieldAttr = (SaythonDbField)propInfos[i].GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                        fieldName = fieldAttr.FieldName;
                        if (dataRow[fieldName] != DBNull.Value)
                        {
        ...
        private static ArrayList GetSelectableFields(Object businessObject)
        {
            ...
            for (Int32 i = 0; i < fieldInfos.Length; i++)
            {
                ...
                if (fi.IsDefined((new SaythonDbField()).GetType(), false))
                {
                    attr = (SaythonDbField)fi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    list.Add(attr);
                }
            }
            ...
            {
                pi = propInfos[i];
                if (pi.IsDefined((new SaythonDbField()).GetType(), false))
                {
                    attr = (SaythonDbField)pi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    list.Add(attr);
                }
            }
            return list;
        ...
        private static Hashtable GetInsertableFields(Object businessObject)
        {
            ...
            for (Int32 i = 0; i < fieldInfos.Length; i++)
            {
                ...
                if (fi.IsDefined((new SaythonDbField()).GetType(), false) && !fi.IsDefined((new SaythonDbReadOnly()).GetType(), false))
                {
                    attr = (SaythonDbField)fi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    if (string.IsNullOrEmpty(attr.LookupTable))
                    {
                        if (fi.IsDefined((new SaythonDbCreatableAutoIncrement()).GetType(), false))
                        {
                        ...
                        else
                        {
                            ...
                            if (v != null)
                            {
                                ...
                                {
                                    if (attr.FieldType == SaythonDbFieldType.Text) value = "'" + value.Replace("'", "''") + "'";
            ...
            {
                pi = propInfos[i];
                if (pi.IsDefined((new SaythonDbField()).GetType(), false) && !pi.IsDefined((new SaythonDbReadOnly()).GetType(), false))
                {
                    attr = (SaythonDbField)pi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    if (pi.IsDefined((new SaythonDbCreatableAutoIncrement()).GetType(), false))
                    {
                        Int32 uniqVal = Guid.NewGuid().GetHashCode();
                        map.Add(attr.FieldName, uniqVal);
        ...
        private static ArrayList GetUpdatableFields(Object businessObject)
        {
            ...
            for (Int32 i = 0; i < fieldInfos.Length; i++)
            {
                ...
                if (fi.IsDefined((new SaythonDbField()).GetType(), false) && !(fi.IsDefined((new SaythonDbReadOnly()).GetType(), false) || fi.IsDefined((new SaythonDbCreatable()).GetType(), false)))
                {
                    attr = (SaythonDbField)fi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    if (string.IsNullOrEmpty(attr.LookupTable))
                    {
            ...
            {
                pi = propInfos[i];
                if (pi.IsDefined((new SaythonDbField()).GetType(), false) && !(pi.IsDefined((new SaythonDbReadOnly()).GetType(), false) || pi.IsDefined((new SaythonDbCreatable()).GetType(), false)))
                {
                    attr = (SaythonDbField)pi.GetCustomAttributes((new SaythonDbField()).GetType(), false)[0];
                    v = pi.GetValue(businessObject, null);
                    if (v != null)
        ...
        private static string GetFieldSelectClause(SaythonDbField attr, string tableName)
        {
